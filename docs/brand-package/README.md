...
- [[12-input-heuristics]] — the input rules' provenance: eleven tagged rules (WCAG criterion / research / house convention), the amendment ledger, and the deliberately-open questions. The operative contract lives in [[04-design-system]] § Form inputs; this file carries the argument.
- [[13-document-surfaces-heuristics]] — use cases, heuristics, patterns and anti-patterns for every document-surface addition, change and pattern (weft#28–#49), grounded in the W3 definitions and the DocT owner rulings; five foundations (the ruled three-channel rule, casing, honest empties, one panel at a time, keyboard / focus / touch) and eleven cross-cutting anti-patterns, seven from the DocT epic pass and four from the accessibility floor
