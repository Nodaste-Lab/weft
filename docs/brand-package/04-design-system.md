...
**Identity colour** — the per-person accent + wash subsystem (`--weft-identity-<name>` / `-wash`, `--weft-on-identity`, and the channel properties an element carrying `data-identity` sets) is defined in [[09-app-primitives]] § Identity colour (token subsystem). **Superseded (DocT owner ruling 2026-09-06, re-affirmed 2026-09-09): colour is semantic only and never names a person, so W4 does not land this subsystem** — the named pairs, `--weft-on-identity` and the `[data-identity]` channel rules stay documented in [[09-app-primitives]] as the history of what was tried, and weft#31 closes without shipping. Identity is carried by the attribution mark (a name, initials or the agent glyph); DocT retired its `--hl-*` pairs under the same ruling. [[13-document-surfaces-heuristics]] carries the rule as ruled.
