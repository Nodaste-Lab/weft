...
**Identity colour** — the per-person accent + wash subsystem (`--weft-identity-<name>` / `-wash`, `--weft-on-identity`, and the channel properties an element carrying `data-identity` sets) is defined in [[09-app-primitives]] § Identity colour (token subsystem). At plan phase W4 (`Nodaste-Lab/weft` #31) the named pairs and `--weft-on-identity` land in this file's `:root` block; the `[data-identity]` channel rules go to `css/weft-components.css`, because `css/weft.css` admits `:root` blocks only. Until then DocT's `--hl-*` pairs stand in.
